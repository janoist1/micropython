...
import time
import ustruct
import ili9341.glcdfont as glcdfont
import framebuf
...
class ILI9341:
    ...
    def set_font(self, font):
        self._font = font if font else glcdfont
    ...
    def init(self):
        for command, data in (
            ...
            (_VMCTRL2, b"\x86"),
            #(_MADCTL, b"\x48"),
            (_MADCTL, b"\x48"), # x08: m4stack landscape, x36: landscape
            (_PIXSET, b"\x55"),
            (_FRMCTR1, b"\x00\x18"),
    ...
    def rectangle(self, x, y, w, h, border, color=None):
      x = min(self.width - 1, max(0, x))
      y = min(self.height - 1, max(0, y))
      w = min(self.width - x, max(1, w))
      h = min(self.height - y, max(1, h))

      self.fill_rectangle(x, y, w, border, color)
      self.fill_rectangle(x, y, border, h, color)
      self.fill_rectangle(w - border, y, border, h, color)
      self.fill_rectangle(x, h - border, w, border, color)
